...
func Test_requestLogger(t *testing.T) {
	...
	if rr.Code != expectedStatusCode {
		t.Errorf("handler returned wrong status code: got %v want %v",
			rr.Code, expectedStatusCode)
	}

	actualLog := logBuffer.String()
	if actualLog != expectedLogString {
		t.Errorf("handler logged unexpected string\ngot:  %q\nwant: %q",
			actualLog, expectedLogString)
	}
}
